import { useRmListPendingApprovals, useRmDecideApproval, getRmListPendingApprovalsQueryKey } from "@workspace/api-client-react";
...
export default function RmApprovals() {
  const [selectedApproval, setSelectedApproval] = useState<any>(null);
  const [decision, setDecision] = useState<'approved' | 'rejected' | null>(null);
  
  const queryClient = useQueryClient();

  const { data: approvals, isLoading } = useRmListPendingApprovals({
    query: { queryKey: getRmListPendingApprovalsQueryKey() }
  });

  const decideApproval = useRmDecideApproval();
  ...
  const handleDecision = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: getRmListPendingApprovalsQueryKey() });
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold tracking-tight">Pending Approvals (Step 1)</h2>
      </div>
              ...
              <TableBody>
                {approvals?.map(approval => (
                  <TableRow key={approval.approval_id}>
                    ...
                    <TableCell className="max-w-[250px] truncate" title={approval.purpose || ''}>
                      {approval.purpose || '-'}
                ...
                {approvals?.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-12 text-muted-foreground">
                      No pending approvals for your resources.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
